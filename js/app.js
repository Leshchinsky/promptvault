...
function showAuthSuccess(message) {
  if (!el.authSuccess) return;
  el.authSuccess.textContent = message;
  el.authSuccess.style.display = message ? 'block' : 'none';
}
...
async function resetPassword() {
  ...
  const { error } = await supabaseClient.auth.resetPasswordForEmail(email, {
    redirectTo: `${window.location.origin}/promptvault/reset-password.html`
  });
...
function renderMobileFilters() {
  ...
  renderSelectOptions(el.mobileTagFilter, getTags(), state.tag, 'All tags');
  ...
  if (!items.length) {
    ...
    if (filtered.length > MOBILE_LIMIT) {
      el.mobilePromptList.insertAdjacentHTML(
        'beforeend',
        `<div class="empty">Showing first ${MOBILE_LIMIT} prompts. Use search, favorites, recent, tags, or sorting to narrow the list.</div>`
      );
    }
  }
...
function renderMobileScreens() {
  const isList = state.mobileScreen === 'list';
  const isDetails = state.mobileScreen === 'details';

  if (el.mobileListScreen) {
    el.mobileListScreen.classList.toggle('active', isList);
  }

  if (el.mobileDetailsScreen) {
    el.mobileDetailsScreen.classList.toggle('active', isDetails);
  }

  if (el.mobileFabBtn) {
    el.mobileFabBtn.style.display = isList ? 'flex' : 'none';
  }
}
...
function bindEvents() {
  ...
  el.mobileTagFilter?.addEventListener('change', e => {
    state.tag = e.target.value;
    renderAll();
  });
